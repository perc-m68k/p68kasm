...
pub fn code_for_statement<M: SymbolMap>(p: Pair<Rule>, symbols: &M) -> (Option<u32>, Vec<u8>) {
    match p.as_rule() {
        Rule::instruction => {
            ...
            let instr = label
                ...
                .unwrap();
            (None, code_for_instr(instr, symbols))
        }
        _ => unreachable!(),
    }
}
...
fn get_mode_reg_extra_for_ea<M: SymbolMap>(p: Pair<Rule>, size: IntSize, symbols: &M) -> (u8, u8, Vec<u8>) {
    match p.as_rule() {
        ...
        Rule::immediate_data => {
			let value = parse_expression(p.into_inner(), symbols);
			(
            0b111,
            0b100,
            match size {
...
fn code_for_instr<M: SymbolMap>(p: Pair<Rule>, symbols: &M) -> Vec<u8> {
    match p.as_rule() {
        Rule::LEA => todo!(),
        Rule::LINK => {
			let mut inner = p.into_inner();
			let an = inner.next().unwrap().into_inner().next().unwrap().as_str().parse::<u8>().unwrap();
			let data = parse_expression(inner.next().unwrap().into_inner(), symbols) as u16;
        ...
        Rule::MOVE => {
            ...
            let size = inner
                ...
                .unwrap_or_default();
            let (src_mode, src_reg, src_extra) = get_mode_reg_extra_for_ea(inner.next().unwrap(), size, symbols);
            let (dst_mode, dst_reg, dst_extra) = get_mode_reg_extra_for_ea(inner.next().unwrap(), size, symbols);
