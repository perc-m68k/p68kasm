...
fn run_passes<'a>(
    ...
    for s in statements.clone() {
        if s.as_rule() == Rule::include {
            ...
            let (label, start_addr, code) =
                code_for_statement(s, pc, &NonFailingMap(&global_data.symbols), &current_file.path.display(), true);
    ...
    for s in statements {
        ...
        if s.as_rule() == Rule::include {
            ...
            let span = s.as_span();
            let (_, start_addr, code) = code_for_statement(s, pc, &global_data.symbols, &current_file.path.display(), false);
            pc = start_addr.unwrap_or(pc);
            ...
            if create_listing {
                for (line, last) in span.lines_span().map(|line| {
                    let line_start = if line.start_pos().line_col().0 == span.start_pos().line_col().0 {
                        span.start_pos()
                    }else{
                        line.start_pos()
                    };
                    ...
                    new_span
                }).filter(|line| !line.as_str().is_empty()).with_last() {
                    if last {
                        global_data.listing.add(current_file.path, line.start_pos().line_col().0, idx);
                    }else {
                        global_data.listing.add_no_code(current_file.path, line.start_pos().line_col().0, idx);
                    }
...
fn run(conf: &Config) {
    ...
    if let Some(listing_path) = &conf.listing {
        ...
        for FileAndContents { file, contents } in &global_data.files {
            f.write_all(
                global_data.listing
    ...
    std::fs::write(
        &conf.out,
        SRec::new(global_data.code_object.iter().map(|(a, b)| (*a, b.as_slice()))).to_string(),
    )
    .unwrap();
}
